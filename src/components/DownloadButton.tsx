...
export default function DownloadButton() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!imgFolder) {
        throw new ThemeDownloadError('ZIP_ERROR', 'Failed to create images folder in ZIP');
      }
      ...
      try {
        ...
        throw new ThemeDownloadError('ZIP_ERROR', `Failed to add image: ${err instanceof Error ? err.message : 'Unknown error'}`);
      }
      ...
      try {
        manifest = {
          ...
          theme: {
            images: {
              theme_ntp_background: images["theme_ntp_background"] || "images/background.png"
            },
        ...
        throw new ThemeDownloadError('MANIFEST_ERROR', `Failed to create manifest: ${err instanceof Error ? err.message : 'Unknown error'}`);
      }

      try {
        zip.file('manifest.json', JSON.stringify(manifest, null, 2));
      } catch (err) {
        throw new ThemeDownloadError('ZIP_ERROR', `Failed to add manifest to ZIP: ${err instanceof Error ? err.message : 'Unknown error'}`);
      }
      ...
      try {
        ...
        throw new ThemeDownloadError('ZIP_ERROR', `Failed to add localization file: ${err instanceof Error ? err.message : 'Unknown error'}`);
      }
      ...
      try {
        ...
        throw new ThemeDownloadError('ZIP_ERROR', `Failed to generate ZIP: ${err instanceof Error ? err.message : 'Unknown error'}`);
      }
      ...
      try {
        ...
        throw new ThemeDownloadError('DOWNLOAD_ERROR', `Failed to download file: ${err instanceof Error ? err.message : 'Unknown error'}`);
      }
    } catch (err) {
  ...
  return (
    <div className="flex flex-col items-end gap-3">
      ...
      >
        ...
        <span>{isLoading ? 'Creating...' : showSuccess ? 'Downloaded!' : 'Download Theme'}</span>
      </button>

      {showError && (
        <div className="w-80 px-4 py-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg shadow-sm">
          <p className="text-sm font-medium text-red-900 dark:text-red-200 mb-1">Download Failed</p>
